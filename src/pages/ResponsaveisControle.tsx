...
import { TaskResponsavelControleRecord } from '@/types/task'
...
export default function ResponsaveisControlePage() {
  const {
  ...
  const [itemEmEdicao, setItemEmEdicao] = useState<TaskResponsavelControleRecord | null>(null)
  ...
  const [itemParaExcluir, setItemParaExcluir] = useState<TaskResponsavelControleRecord | null>(null)
  const [excluindo, setExcluindo] = useState(false)

  // Filtragem e ordenação alfabética
  const listaFiltrada = useMemo(() => {
    let list = [...responsaveisControle]
    if (busca.trim()) {
  ...
  const handleOpenEditar = (item: TaskResponsavelControleRecord) => {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    if (nomeTrim.length > 255) {
      setFormError('O nome do responsável deve ter no máximo 255 caracteres.')
      return
    }
    ...
    setSalvando(true)
    try {
      await saveResponsavelControle({
        id: itemEmEdicao?.id,
        nome: nomeTrim,
        ativo: ativoForm,
      })
      toast({
        title: itemEmEdicao ? 'Responsável atualizado' : 'Responsável cadastrado',
        description: `"${nomeTrim}" salvo com sucesso.`,
      })
      setModalOpen(false)
    } catch (err: any) {
      console.error('Erro ao salvar Responsável pelo Controle:', err)
      if (
        err?.code === '23505' ||
        err?.message?.includes('23505') ||
        err?.message?.includes('Já existe um Responsável equivalente')
      ) {
        setFormError('Já existe um Responsável equivalente a este.')
      } else {
  ...
  const handleToggleAtivo = async (item: TaskResponsavelControleRecord) => {
    try {
      await toggleResponsavelControleAtivo(item.id, !item.ativo)
      toast({
        title: item.ativo ? 'Responsável desativado' : 'Responsável ativado',
  ...
  const handleOpenExcluir = (item: TaskResponsavelControleRecord) => {
  ...
  const handleConfirmExclusao = async () => {
    ...
    setExcluindo(true)
    try {
      await excluirResponsavelControle(itemParaExcluir.id)
      toast({
      ...
      toast({
        ...
        description: err?.message || 'Falha ao excluir o Responsável.',
      })
    } finally {
      setExcluindo(false)
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full min-w-0">
      ...
      <PageHeader
        ...
        subtitle="Administração dos responsáveis exclusivos vinculados aos casos jurídicos"
        actions={
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => refreshResponsaveisControle()}
              disabled={loading}
      ...
      <div className="bg-card border border-border rounded-2xl p-4 shadow-card flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground self-end sm:self-center">
          <span>
            Total: <strong className="text-foreground">{responsaveisControle.length}</strong>
          </span>
          <span>•</span>
          <span className="text-emerald-600 dark:text-emerald-400 font-medium">
            Ativos: {responsaveisControle.filter((r) => r.ativo).length}
          </span>
          <span>•</span>
          <span className="text-muted-foreground">
            Inativos: {responsaveisControle.filter((r) => !r.ativo).length}
          </span>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-muted-foreground flex items-center justify-center gap-2 text-sm">
            ...
            <span>Carregando Responsáveis pelo Controle...</span>
          </div>
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <div className="space-y-1">
              <p className="font-semibold text-foreground text-sm">
                {busca ? 'Nenhum resultado encontrado' : 'Nenhum Responsável cadastrado'}
              </p>
              <p className="text-xs text-muted-foreground">
                {busca
                  ? 'Tente ajustar sua busca por outros termos.'
                  : 'Cadastre o primeiro responsável pelo controle usando o botão acima.'}
              </p>
            </div>
            {busca && (
              <Button
              ...
              >
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Somente responsáveis cadastrados nesta tabela podem ser vinculados a um caso.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                  <Button
                    ...
                    title={item.ativo ? 'Desativar este responsável' : 'Ativar este responsável'}
                  >
        ...
        <DialogContent className="max-w-md rounded-2xl p-6">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-foreground flex items-center gap-2">
              ...
              <span>
                {itemEmEdicao ? 'Editar Responsável pelo Controle' : 'Adicionar Responsável'}
              </span>
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              Cadastre o nome do responsável usado nos casos (ex: RICCI, NATURA, correspondentes).
            </DialogDescription>
          </DialogHeader>

          <form onSubmit={handleSalvar} className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="nome-resp-input" className="text-xs font-semibold text-foreground">
              ...
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                ...
                <span>{nomeForm.trim().length}/255</span>
              </div>
            </div>
      ...
      <DeleteConfirmDialog
        ...
        title="Excluir Responsável pelo Controle?"
